...
public partial class CsWin32GeneratorTests
{
    ...
    private IReadOnlyList<ISourceGenerator> GetAvailableSourceGenerators()
    {
        ...
        foreach (string analyzerPath in analyzers)
        {
            ...
            {
                // Some analyzer assemblies might not be loadable or might not contain types
                this.Logger.WriteLine($"Could not examine analyzer assembly {analyzerPath}: {ex.Message}");
            }
        }
