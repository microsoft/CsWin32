...
{
    private static readonly Lazy<ReferenceAssemblies> LazyDefaultReferenceAssemblies = new(() =>
    {
#if NET5_0
...
#error Not supported.
    ...
    private static ReferenceAssemblies DefaultReferenceAssemblies => LazyDefaultReferenceAssemblies.Value;
...
";
        await new VerifyTest
        {
            TestState =
            {
                ...
                GeneratedSources =
                {
                    LoadExpectedOutput("DISPLAYCONFIG_SCANLINE_ORDERING.g.cs"),
                },
            },
        }.RunAsync();
    }
...
";
        await new VerifyTest
        {
            TestState =
            {
                ...
                GeneratedSources =
                {
                    LoadExpectedOutput("FILE_ACCESS_FLAGS.g.cs"),
                },
            },
        }.RunAsync();
    }
...
";
        await new VerifyTest
        {
            TestState =
            {
                ...
                {
                    LoadExpectedOutput("BOOL.g.cs"),
                    LoadExpectedOutput("Delegates.g.cs"),
                    LoadExpectedOutput("HWND.g.cs"),
                    LoadExpectedOutput("LPARAM.g.cs"),
                },
            },
        }.RunAsync();
    }
...
";
        await new VerifyTest
        {
            TestState =
            {
                ...
                GeneratedSources =
                {
                    LoadExpectedOutput("HDC.g.cs"),
                    LoadExpectedOutput("HWND.g.cs"),
                    LoadExpectedOutput("PInvoke.User32.g.cs"),
                },
            },
        }.RunAsync();
    }
        ...
        await new VerifyTest
        {
            TestState =
            {
                ...
                GeneratedSources =
                {
                    LoadExpectedOutput("BOOL.g.cs"),
                    LoadExpectedOutput("FILE_ACCESS_FLAGS.g.cs"),
                    LoadExpectedOutput("FILE_CREATION_DISPOSITION.g.cs"),
                    LoadExpectedOutput("FILE_FLAGS_AND_ATTRIBUTES.g.cs"),
                    LoadExpectedOutput("FILE_SHARE_MODE.g.cs"),
                    LoadExpectedOutput("HANDLE.g.cs"),
                    LoadExpectedOutput("PCWSTR.g.cs"),
                    LoadExpectedOutput("PInvoke.Kernel32.g.cs"),
                    LoadExpectedOutput("PWSTR.g.cs"),
                    LoadExpectedOutput("SECURITY_ATTRIBUTES.g.cs"),
                },
            },
        }.RunAsync();
    }

    private static (Type GeneratorType, string FileName, string Content) LoadExpectedOutput(string name)
    {
        using Stream? stream = typeof(GeneratorTests).Assembly.GetManifestResourceStream(name);
        if (stream is null)
        {
            throw new ArgumentException($"Could not find resource '{name}'", nameof(name));
        }

        using var reader = new StreamReader(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks: false, bufferSize: 4096, leaveOpen: true);
        return (typeof(SourceGenerator), name, reader.ReadToEnd());
    }
