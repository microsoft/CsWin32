...
namespace Microsoft.Windows.CsWin32
{
    ...
    public class Generator : IDisposable
    {
        ...
        private const string SystemTextStringBuilder = "System.Text.StringBuilder";
...
";
        ...
        private static readonly IdentifierNameSyntax StringBuilderTypeSyntax = IdentifierName(SystemTextStringBuilder);
...
"));
        ...
        private IEnumerable<MethodDeclarationSyntax> CreateFriendlyOverloads(MethodDefinition methodDefinition, MethodDeclarationSyntax externMethodDeclaration, string declaringTypeName, bool isStatic)
        {
            ...
            foreach (ParameterHandle paramHandle in methodDefinition.GetParameters())
            {
                ...
                var nativeTypeInfo = this.GetNativeTypeInfo(param.GetCustomAttributes());
                    ...
                    && !(ptrType.ElementType is IdentifierNameSyntax id && IsInterface(id.Identifier.ValueText)))
                {
                    ...
                    var isArray = nativeTypeInfo.UnmanagedType switch
                    {
                        UnmanagedType.LPWStr => true,
                        UnmanagedType.LPStr => true,
                        UnmanagedType.LPTStr => true,
                        UnmanagedType.LPArray => true,
                        _ => false,
                    };
                    ...
                    if (isArray)
                    {
                        ...
                        if (nativeTypeInfo.SizeParamIndex.HasValue
                            && !(externMethodDeclaration.ParameterList.Parameters[nativeTypeInfo.SizeParamIndex.Value].Type is PointerTypeSyntax)
                            && !isPointerToPointer)
                        {
                            signatureChanged = true;

                            if (lengthParamUsedBy.TryGetValue(nativeTypeInfo.SizeParamIndex.Value, out int userIndex))
                            {
                            ...
                            {
                                lengthParamUsedBy.Add(nativeTypeInfo.SizeParamIndex.Value, param.SequenceNumber - 1);
                            }
                            ...
                            if (!(parameters[nativeTypeInfo.SizeParamIndex.Value].Type is PredefinedTypeSyntax { Keyword: { RawKind: (int)SyntaxKind.IntKeyword } }))
                            {
                                sizeArgExpression = CastExpression(parameters[nativeTypeInfo.SizeParamIndex.Value].Type!, sizeArgExpression);
                            }

                            arguments[nativeTypeInfo.SizeParamIndex.Value] = Argument(sizeArgExpression);
                        }
                        else if (nativeTypeInfo.SizeConst.HasValue && !isPointerToPointer)
                        {
                            ...
                            leadingStatements.Add(IfStatement(
                                BinaryExpression(
                                    ...
                                    GetSpanLength(origName),
                                    LiteralExpression(SyntaxKind.NumericLiteralExpression, Literal(nativeTypeInfo.SizeConst.Value))),
                                ThrowStatement(ObjectCreationExpression(IdentifierName(nameof(ArgumentException))).WithArgumentList(ArgumentList()))));
                        }
                        else if (nativeTypeInfo.IsNullTerminated && externParam.Type is PointerTypeSyntax { ElementType: PredefinedTypeSyntax { Keyword: { RawKind: (int)SyntaxKind.CharKeyword } } })
                        {
                ...
                else if (nativeTypeInfo.SizeParamIndex.HasValue
                         && !(externMethodDeclaration.ParameterList.Parameters[nativeTypeInfo.SizeParamIndex.Value].Type is PointerTypeSyntax)
                         && externParam.Type is IdentifierNameSyntax { Identifier: { ValueText: SystemTextStringBuilder } })
                {
                    signatureChanged = true;
                    lengthParamUsedBy.Add(nativeTypeInfo.SizeParamIndex.Value, param.SequenceNumber - 1);

                    IdentifierNameSyntax origName = IdentifierName(externParam.Identifier.ValueText);
                    ExpressionSyntax sizeArgExpression = MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, origName, IdentifierName(nameof(StringBuilder.Capacity)));
                    arguments[nativeTypeInfo.SizeParamIndex.Value] = Argument(sizeArgExpression);
                }
            }
        ...
        private NativeTypeInfo GetNativeTypeInfo(CustomAttributeHandleCollection attrs)
        {
            foreach (CustomAttributeHandle attHandle in attrs)
            {
                CustomAttribute att = this.mr.GetCustomAttribute(attHandle);
                ...
                var args = att.DecodeValue(this.customAttributeTypeProvider);
                var unmanagedType =
                    args.FixedArguments[0].Value is { } value ? (UnmanagedType)value : default;

                var isNullTerminated =
                    args.NamedArguments.Any(a => a.Name == "IsNullTerminated" && a.Value is bool value && value);
                var sizeParamIndex = (short?)args.NamedArguments.FirstOrDefault(a => a.Name == "SizeParamIndex").Value;
                var sizeConst = (int?)args.NamedArguments.FirstOrDefault(a => a.Name == "SizeConst").Value;

                return new NativeTypeInfo(unmanagedType, isNullTerminated, sizeParamIndex, sizeConst);
            }
        ...
        private ParameterSyntax CreateParameter(MethodSignature<TypeSyntax> methodSignature, Parameter parameter)
        {
            ...
            var isIn = (parameter.Attributes & ParameterAttributes.In) == ParameterAttributes.In;
            var isOut = (parameter.Attributes & ParameterAttributes.Out) == ParameterAttributes.Out;
            var isOptional = (parameter.Attributes & ParameterAttributes.Optional) == ParameterAttributes.Optional;
            ...
            SyntaxKind directionalModifier =
                isIn && isOut ? SyntaxKind.RefKeyword :
                isIn ? SyntaxKind.InKeyword :
                isOut ? SyntaxKind.OutKeyword :
                default;
            if (parameterInfo.Type is PredefinedTypeSyntax { Keyword: { RawKind: (int)SyntaxKind.StringKeyword } } && isOut)
            {
                // Out strings must use StringBuilder
                parameterInfo = (StringBuilderTypeSyntax, parameterInfo.MarshalAsAttribute);
                attributes = AttributeList(); // remove in/out/ref attributes
            }
            else if (parameterInfo.Type is PointerTypeSyntax { ElementType: TypeSyntax ptrElem1 } && this.IsManagedType(ptrElem1))
            {
        ...
        private (TypeSyntax Type, AttributeSyntax? MarshalAsAttribute) ReinterpretMethodSignatureType(TypeSyntax originalType, CustomAttributeHandleCollection customAttributes)
        {
            ...
            var nativeTypeInfo = this.GetNativeTypeInfo(customAttributes);

            if (!nativeTypeInfo.UnmanagedType.HasValue)
            {
                return (originalType, null);
            }

            var unmanagedType = nativeTypeInfo.UnmanagedType.Value;
            if (unmanagedType is UnmanagedType.Bool)
            {
                return (PredefinedType(Token(SyntaxKind.BoolKeyword)), MarshalAs(unmanagedType));
            }

            var isString =
                unmanagedType switch
                {
                    UnmanagedType.LPWStr => originalType is PointerTypeSyntax { ElementType: PredefinedTypeSyntax { Keyword: { RawKind: (int)SyntaxKind.UShortKeyword } } },
                    UnmanagedType.LPStr => originalType is PointerTypeSyntax { ElementType: PredefinedTypeSyntax { Keyword: { RawKind: (int)SyntaxKind.SByteKeyword } } },
                    _ => false,
                };

            if (isString)
            {
                return (PredefinedType(Token(SyntaxKind.StringKeyword)), MarshalAs(unmanagedType));
            }
