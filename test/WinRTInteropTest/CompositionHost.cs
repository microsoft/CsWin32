...
namespace WinRTInteropTest
{
    ...
    internal class CompositionHost
    {
        ...
        internal void AddElement(float size, float x, float y)
        {
            ...
            var visuals = ((ContainerVisual)this.desktopWindowTarget.Root).Children;
            var visual = this.compositor.CreateSpriteVisual();

            var element = this.compositor.CreateSpriteVisual();

            Windows.UI.Color color = new Windows.UI.Color() { A = 255, R = (byte)this.random.Next(256), G = (byte)this.random.Next(256), B = (byte)this.random.Next(256) };
            ...
            var bottom = 600.0f - element.Size.Y;
        ...
        {
            var root = this.compositor.CreateContainerVisual();
